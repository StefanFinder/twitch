...
// class we use to collect all videos, streams and clips for a specific game
public record TypeGroupedItemList(
        List<ItemEntity> streams,
        List<ItemEntity> videos,
        List<ItemEntity> clips
) {
    // here we just get the data from db, now we want to filter
    ...
    // here we just get the data from twitch, and we want to fill the corresponding list
